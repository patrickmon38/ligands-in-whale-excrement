---
title: Code for figures in *'Organic ligands in whale excrement support iron availability and reduce copper toxicity to the surface ocean'* by Monreal et al.
output:
  html_document:
    df_print: paged
...
tab1 = as.data.frame(read.xlsx(github_data, sheet = 2, startRow = 1))
tab1
tab2 = as.data.frame(read.xlsx(github_data, sheet = 3, startRow = 1))
tab2
#######################################################
```
## Data for Fig. 2, with ICPMS data used for Fig. S1 and ESIMS data used for Fig. S5 and Fig. S8 as well
```{r, message = FALSE, warning = FALSE}
#######################################################
...
ggplot(dfe_frame) +
  ...
  theme(axis.title.x = element_blank(),
        ...
        axis.text.x = element_text(angle = 45, face = "bold", hjust = 1, color = c("black", "black","gray40","gray40","gray40","black","black","black")),
...
ggplot() +
  ...
  scale_fill_manual(values = c("#9986A5","#9986A5","#79402E","#AA9486")) +
  ...
  theme(axis.title.x = element_blank(),
        ...
        axis.text.x = element_text(angle = 45, face = "bold", hjust = 1, color = c("black", "black","gray40","gray40","gray40")),
        axis.text.y.left = element_text(face = "bold", color = "black"),
...
```{r echo=TRUE, message = FALSE, warning = FALSE, fig.width=3, fig.height=6}
...
ggplot(dcu_lig_frame) +
  ...
  theme(axis.title.x = element_blank(),
        axis.title.y.left = element_markdown(),
        axis.text.x = element_text(angle = 45, face = "bold", hjust = 1, color = c("black", "black")),
        legend.position = "none",
        axis.text.y.left = element_text(face = "bold", color = "black"))
```
...
```{r echo=TRUE, message = FALSE, warning = FALSE, fig.width=7, fig.height=8.5}
#Reformatting data
...
cols <- matrix("black", nrow(tab1), ncol(tab1))
cols[1:5,1] <- c("black", "black","gray40","gray40","gray40")

tt1 = ttheme_default()
tt1$core$fg_params <- list(fontface=matrix(c(2,3,1,1,1), ncol=ncol(tab1),nrow=nrow(tab1),byrow=TRUE))
tt1$core$fg_params$col = cols
tt1$core$bg_params$col = NA
tt1$colhead$bg_params$col = NA

t1 = tableGrob(tab1, rows = NULL, theme = tt1)

t1 <- gtable_add_grob(t1,
                     grobs = rectGrob(gp = gpar(fill = NA, lwd = 4)),
                     t = 1, l = 1, r = ncol(t1))

tt2 = ttheme_default(parse = TRUE)
tt2$core$fg_params <- list(fontface=matrix(c(2,1,1,1,1), ncol=ncol(tab2),nrow=nrow(tab2),byrow=TRUE))
tt2$core$fg_params$col = cols
tt2$core$bg_params$col = NA
tt2$colhead$bg_params$col = NA
t2 = tableGrob(tab2, theme = tt2, rows = NULL)
...
```{r echo=TRUE, message = FALSE, warning = FALSE, fig.width=17, fig.height=22, warning=FALSE}
...
```{r echo=TRUE, message = FALSE, warning = FALSE, fig.width=9, fig.height=10, warning=FALSE}
...
```{r echo=TRUE, message = FALSE, warning = FALSE, fig.width=8, fig.height=7.5, warning=FALSE}
...
```{r echo=TRUE, message = FALSE, warning = FALSE, fig.width=8.5, fig.height=5.5, warning=FALSE}
...
```{r echo=TRUE, message = FALSE, warning = FALSE, fig.width=12.75, fig.height=16.5, warning=FALSE}
...
#### Figure S4 was a photograph of sample filtrate, with no code.

## Figure S5

#### Extracted ion chromatogram (EIC) of Cu ligand (m/z = 654.24751) with multiple peaks.

```{r echo=TRUE, message = FALSE, warning = FALSE, fig.width=6, fig.height=5, warning=FALSE}
...
```{r echo=TRUE, message = FALSE, warning = FALSE, fig.width=14.875, fig.height=19.25, warning=FALSE}
mz_ms2_1 = 650.21622
mz_rt_1 = 26.46
apo_ms2_1 = 589.30212
apo_rt_1 = 24.41
...
#### Figure S7 are chemical structures generated with ChemDraw.

## Figure S8

#### Extracted ion chromatograms (EICs) of domoic acid ion

```{r echo=TRUE, message = FALSE, warning = FALSE, fig.width=8.5, fig.height=13, warning=FALSE}
DA_mass = 312.144714
...
#### Map of sample collection areas

```{r}
library(ggOceanMaps, quietly = TRUE)
library(ggspatial, quietly = TRUE)
library(ggforce, quietly = TRUE)

# We altered the bathymetric file that is used in this chunk to be courser resolution, which makes this chunk easier to run for the purposes of this file. bathy.style = "rcb" (a much finer-resolution bathymetry) was used when generating the actual figure S9

suppyy = basemap(limits = c(-125,-121,35.5,38.5), bathy.style = "raster_binned_blues", grid.col = "black") +
  #ggspatial::geom_spatial_point(aes(x = -122.0968, y = 36.801), color = "darkred") +
  #ggspatial::geom_spatial_point(aes(x = -122.879087, y = 37.757581), color = "darkred")+
  #ggspatial::geom_spatial_point(aes(x = -121.977, y = 36.4198), color = "darkred") +
  labs(x = NULL, y = NULL, title = "California Current Samples", subtitle = "CRC7, CRC210, and MSS1") +
  ggspatial::annotation_scale(location = "bl", text_col = "white") +
  geom_ellipse(aes(x0 = -123.1, y0 = 37.825, a = 0.2, b = 0.12, angle = 60), fill = "salmon", alpha = 0.25, color = "black") +
  ggspatial::geom_spatial_text(aes(x = -123.1, y = 37.825, label = "CRC7"), angle = 30, size = 3, fontface = "bold") +
  geom_ellipse(aes(x0 = -122.0968, y0 = 36.801, a = 0.15, b = 0.12, angle = 0), fill = "salmon", alpha = 0.25, color = "black") +
  ggspatial::geom_spatial_text(aes(x = -122.0968, y = 36.801, label = "CRC\n210"), size = 2.6, fontface = "bold") +
  geom_ellipse(aes(x0 = -122.05, y0 = 36.4198, a = 0.15, b = 0.12, angle = 0), fill = "salmon", alpha = 0.25, color = "black") +
  ggspatial::geom_spatial_text(aes(x = -122.05, y = 36.4198, label = "MSS1"), size = 2.6, fontface = "bold") +
  ggspatial::geom_spatial_label(aes(x = -123.65, y = 37.9, label = "07/02/2020"), size = 3, fontface = "bold") +
  ggspatial::geom_spatial_label(aes(x = -122.65, y = 36.9, label = "08/29/2018"), size = 3, fontface = "bold") +
  ggspatial::geom_spatial_label(aes(x = -122.55, y = 36.5, label = "08/28/2018"), size = 3, fontface = "bold") +
  theme(axis.text.x = element_text(color = "black", face = "bold", size = 11),
        axis.text.y = element_text(color = "black", face = "bold", size = 11))

suppxx = basemap(limits = c(-100, 0, -90, -62.5), bathy.style = "raster_binned_blues", grid.col = "black") +
  ggspatial::geom_spatial_point(aes(x = -64.0538, y = -64.7743), color = "black", size = 9, shape = 21, fill = "salmon", alpha = 0.25) +
  ggspatial::geom_spatial_point(aes(x = -64.0538, y = -64.7743), color = "black", size = 9, shape = 1) +
  ggspatial::geom_spatial_text(aes(x = -7.5, y = -59, label = "60°S"), color = "black", angle = 7.5) +
  ggspatial::geom_spatial_text(aes(x = -7.5, y = -69, label = "70°S"), color = "black", angle = 7.5) +
  ggspatial::geom_spatial_text(aes(x = -7.5, y = -79, label = "80°S"), color = "black", angle = 7.5) +
  ggspatial::geom_spatial_text(aes(x = -125, y = -85, label = "135°W"), color = "black", angle = 45) +
  ggspatial::geom_spatial_text(aes(x = -80, y = -85, label = "90°W"), color = "black", angle = 0) +
  ggspatial::geom_spatial_text(aes(x = -35, y = -85, label = "45°W"), color = "black", angle = -45) +
  ggspatial::geom_spatial_label(aes(x = -55, y = -69, label = "MN19: 01/31/2019"), size = 3, fontface = "bold") +
  ggspatial::geom_spatial_label(aes(x = -58, y = -71, label = "MN20: 03/02/2020"), size = 3, fontface = "bold") +
  labs(title = "Antarctic Peninsula Samples", subtitle = "MN19 and MN20") +
...
library(cowplot)
map_plot = plot_grid(suppyy, suppxx, 
          nrow = 1,
          labels = c("(A)","(B)"),
          label_size = 16,
          align = "hv", 
...
```{r echo=TRUE, message = FALSE, warning = FALSE, fig.width=8, fig.height=10, warning=FALSE}
g1 = ggplot(data = MN19 %>% filter(treatment != "SPIKE")) +
  aes(x = potential, y = Y, color = treatment) +
  geom_line(size = 1) +
  scale_color_manual(values = stepped3(16)[16:2], labels = c("0 nM",
...
```{r echo=TRUE, message = FALSE, warning = FALSE, fig.width=12, fig.height=7, warning=FALSE}
